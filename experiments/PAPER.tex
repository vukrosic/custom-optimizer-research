...
\title{Adaptive Attention Selection: A Proof-of-Concept for Dynamic Routing in Linear Transformers}
\author{}
\date{}
...
\begin{enumerate}
    \item \textbf{$f$ (The Actual Workload):} What fraction of tokens did we \textit{actually} assign to each expert?
    \begin{itemize}
        \item \textit{Example:} If we have 100 tokens and send 90 to Linear, $f = [0.9, 0.1]$.
    \end{itemize}
    \item \textbf{$P$ (The Manager's Bias):} What was the average probability (confidence) the router had for each expert?
    \begin{itemize}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{results_dynamic/routing_comparison.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{results_dynamic/layer_selection_over_time.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{results_dynamic/final_routing_distribution.png}
