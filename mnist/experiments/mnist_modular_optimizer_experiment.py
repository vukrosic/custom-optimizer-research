"""
MNIST Modular Optimizer Experiment: Different Optimizers for Different Model Parts
...
class ModularMNISTNet(nn.Module):
    """MLP for MNIST with named layers for modular optimization."""
...
def train_model(config, train_loader, test_loader, device, n_epochs=10):
    ...
    model = ModularMNISTNet(hidden_sizes=[512, 256, 128]).to(device)
...
def plot_results(results, save_prefix='mnist_modular'):
    ...
    plt.suptitle('MNIST Modular Optimizer Experiment:\nDifferent Optimizers for Different Model Parts', 
...
def print_summary(results):
    ...
    print("\n" + "="*80)
    print("EXPERIMENT SUMMARY: MNIST Modular Optimizer")
...
def main():
    parser = argparse.ArgumentParser(description='MNIST Modular Optimizer Experiment')
    ...
    # Plotting
    print("\n📊 Generating plots...")
    plot_results(results, save_prefix='mnist_modular')
