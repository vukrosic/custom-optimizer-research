...
class DepthAwareLLMNet(nn.Module):
...
def train_with_strategy(strategy, train_loader, test_loader, device, 
    ...
    model = DepthAwareLLMNet(hidden_sizes=[512, 256, 128]).to(device)
...
def run_experiment(n_epochs=10, batch_size=128):
    ...
    train_loader, test_loader = get_llm_loaders(batch_size=batch_size)
